...
- **并行的是调查，不是实现。** 搜证、盘点、交叉核验可以放心扇出给只读 agent；**写代码留在主线程**，或至多一个写手
...
```mermaid
flowchart TD
    M["主线程<br/>规划 · 实现 · 验证 · 集成"]
    M -->|只读扇出| A1["Agent：搜证 A"]
    M -->|只读扇出| A2["Agent：搜证 B"]
    M -->|只读扇出| A3["Agent：反方审视"]
    A1 & A2 & A3 -->|"证据（非结论）"| M
```

### 为什么有效

调查是幂等的、无副作用的，天然适合并行；实现有状态、有顺序依赖，并行的协调成本高于收益。「不信子 agent 自报绿」（模式四）和「实现不外派」是同一枚硬币的两面：**你可以外包劳动，不能外包判断。**
...
想立刻跑起来、不想读完全文？→ [`starter/`](starter/) 是模式一的最小可落地版：一段 `CLAUDE.md` 规则 + 一个 fail-open 的 hook，约 2 分钟装好，AI 就开始自己维护工作日志。**这是「工作日志怎么自动生成」的真实答案**——不是魔法，是「规则常驻 + hook 兜底 + 恢复读回」。
