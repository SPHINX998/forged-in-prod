...
1. **SPHINX998** — 发起人、第一共创人，业务裁决与最终口径 owner
2. **OpenAI Codex** — 工作流重构、长任务编排与工程验证共创
3. **Anthropic Claude** — 方法论梳理、反方审视与早期实现共创
...
| 🕸️ **长任务失控** | 一小时任务开工就盲目扇出，关键路径没缩短，主线程反而淹没在十份报告和重复核验里 |

七堵墙，七个模式。**每个模式都先讲事故，再讲规则**——因为顺序反了你就不会信。
...
## 模式七：长任务编排 —— 先排关键路径，再开工

### 事故现场

一次预计超过一小时的重构，开工就拆成十路并行。八路结果暂时用不上，真正控制墙钟时间的热点文件和最终构建仍只能串行；主线程随后花掉大量上下文阅读十份报告，并把关键检查重新跑了一遍。看起来很忙，任务却没有更快。

### 规则

- **长任务不自动扇出。** 先做一次廉价侦察，只产出：当前阶段验收/决策门、关键路径、写 owner、本阶段必要分支和验证路径；调研本身仍须充分
- **先压关键路径。** 并行只能缩短可并行的那一段；如果墙钟主要耗在一个热点文件、一次构建或同一前置事实上，直接串行
- **以阶段决策门定义“必要”。** 拿不到某分支结果就无法关闭当前阶段门，才算本阶段必要；主线能不靠它继续收口的分支延后，不投机预扇
- **按需扇出，不按规模扇出。** 至少两个必要、前置稳定、写目标不冲突、可独立验证的分支，且并行收益覆盖派发/汇总/消费/核验开销时才并行
- **详细结果落 runtime，主线程消费短回执。** artifact 保留完整证据和过程；回执声明 baseline、覆盖分母、失效条件与可复核入口。消费前评估失效条件，无法评估就重验
- **验证左移到自然集成边界。** 分支各自跑目标检查；每完成一个可集成闭环就验证，最终整合后真实驱动组合目标路径。不要每路重跑全套，也不要等全部做完才第一次验
- **失败不能静默丢覆盖。** 分支超时或失败必须明确当前决策门继续阻塞，或带着已声明的覆盖缺口继续
- **协调 agent、怀疑者 agent、分层汇总按需。** 只有分支多、同质、冲突或高风险时才值得增加这一层，不是默认仪式

### 为什么有效

它同时守住两个边界：用 Amdahl 定律避免“为并行而并行”，用短回执和消费点失效检查控制主线程上下文；验证仍在主线程的真实组合路径上闭环。**压缩的是调查和等待的墙钟时间，不是把调研、证据或验证做薄。**

📄 配套模板：[templates/agent-receipt-template.md](templates/agent-receipt-template.md)
...
| **第一次做一小时以上长任务** | 先定位阶段决策门与关键路径；只扇出必要且净收益为正的分支，统一用短回执收口 |
